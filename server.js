...
document.addEventListener('DOMContentLoaded', () => {
    ...
    function generatePDF(teacherName, dept, designation, empId) {
        ...
        doc.text(`Faculty Name: ${teacherName}`, 10, detailsStartY);
        doc.text(`Department: ${dept}`, 10, detailsStartY + lineHeight + 10); // Increased spacing
        doc.text(`Designation: ${designation}`, 10, detailsStartY + lineHeight * 2 + 20); // Increased spacing
        doc.text(`Employee ID: ${empId}`, 10, detailsStartY + lineHeight * 3 + 30); 
        doc.setFontSize(20);// Increased spacing
        doc.text('Acadamic Overview :', 10, detailsStartY + lineHeight * 4 + 40); // Added line for Acad
        // Add some spacing before the tables
        let startY = detailsStartY + lineHeight * 5 + 50; // Extra spacing before tables
        ...
        if (tables.length > 0) {
            tables.forEach((table, index) => {
               // Assume you set data-year attribute in your HTML
                ...
                doc.autoTable({
                    ...
                    margin: { top: startY, bottom: 30 },
                    tableWidth: 'auto',
                    pageBreak: 'auto',
                    didParseCell: function(data) {
                        // Handle page break if the table doesn't fit on the page
                ...
                // Check for page break after each table
            ...
            // Add signature placeholders below the last table
            const finalY = startY + 40; // Adjusted spacing for signatures
            doc.setFontSize(12);
            
// Add signature placeholders below the last table

// Position the signatures on opposite sides
const leftX = 40; // X position for the COE signature
const rightX =  370; // X position for the Director's signature

doc.text('Signature of COE', leftX, finalY);
doc.text('Signature of Director', rightX, finalY);
